...
def on_message(mqttc, obj, msg):
    ...
    points = \
        [
            {
                "measurement": msg_json["app_id"],
                "time": msg_json["metadata"]["time"],
                "tags": {
                    "app_id": msg_json["app_id"],
                    "dev_id": msg_json["dev_id"],
                    "hardware_serial": msg_json["hardware_serial"],
                },
    ...
    res = influx_client.write_points(points)
    logger.info(res)
...
influx_client = InfluxDBClient(host, port, user, password)
